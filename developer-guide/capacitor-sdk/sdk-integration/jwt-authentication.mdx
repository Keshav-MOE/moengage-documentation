...
You can enable JWT authentication during [SDK initialization](https://www.moengage.com/docs/developer-guide/ios-sdk/sdk-integration/basic/sdk-initialization) by configuring the by configuring the **networkConfig** property on the **MoEngageSDKConfig** object.
<CodeGroup>
...
sdkConfig.networkConfig = MoEngageNetworkAuthorizationConfig(isJwtEnabled: true, shouldEnableForAllUsers: true)
let config = MoEngageSDKInitializationConfig(sdkConfig: sdkConfig)
MoECapacitorInitializer.sharedInstance.initializeInstance(withConfig: config)
```
...
`export interface MoEJwtAuthenticationData {
  token: string;
