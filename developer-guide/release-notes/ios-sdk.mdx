...
| :-- | :-- | :-- |
| 10.12.0| <ul><li>Core</li><li><Badge color="green" size="sm">InApp</Badge></li><li>RichNotification</li><li>CampaignsCore</li><li>LiveActivity</li><li>Messaging</li><li>Cards</li><li>Inbox</li><li>Geofence</li><li>RealTimeTrigger</li><li><Badge color="green" size="sm">Personalize</Badge></li></ul> | swift = 5.0.0<br />iOS = 13.0<br />tvOS = 13.0 |
...
| SDK Version | Modules | Dependencies |
| :-- | :-- | :-- |
| 10.11.0 | <ul><li>Core</li><li>InApp</li><li>RichNotification</li><li>CampaignsCore</li><li>LiveActivity</li><li>Messaging</li><li>Cards</li><li>Inbox</li><li>Geofence</li><li>RealTimeTrigger</li><li><Badge color="green" size="sm">Personalize</Badge></li></ul> | swift = 5.0.0<br />iOS = 13.0<br />tvOS = 13.0 |
...
| SDK Version | Modules | Dependencies |
| :-- | :-- | :-- |
| 10.10.2 | <ul><li>Core</li><li><Badge color="green" size="sm">InApp</Badge></li><li>RichNotification</li><li>CampaignsCore</li><li>LiveActivity</li><li>Messaging</li><li>Cards</li><li>Inbox</li><li>Geofence</li><li>RealTimeTrigger</li></ul> | swift = 5.0.0<br />iOS = 13.0<br />tvOS = 13.0 |
...
- **Introduced in:** 10.09.0
- **Affected versions:** 10.09.0, 10.10.0, and 10.10.1
- **Who is affected:** Only apps that do not integrate the `MoEngageLiveActivity` module
- **Resolution:** Upgrade to 10.10.2 or later
...
<Badge color="orange">Fixes:</Badge>\
Fixed Objective-C nullability warnings in module headers. No changes required in your project.
...
- **JWT v2:** Updated JWT implementation to v2 for enhanced security. If you use JWT-based SDK authentication, refer to [JWT Authentication](/developer-guide/ios-sdk/sdk-integration/advanced/jwt-authentication) for the updated configuration.
- **Remote data center host update:** Added support for remotely updating the data center host without an app release. This is configured server-side by MoEngage; no changes are required in your integration.
...
<Badge color="orange">Fixes:</Badge>\
Fixed an internal mapping issue in the InApp statistics reporting API. No changes required in your integration.
...
<Card title="Release Summary">
  | SDK Version | Modules | Dependencies |
  | :-- | :-- | :-- |
  | 10.05.0 | <ul><li>Core</li><li>Messaging</li><li>InApp</li><li>Cards</li><li>Inbox</li><li>Geofence</li><li>RichNotification</li><li>RealTimeTrigger</li></ul> | swift = 5.0.0<br />iOS = 13.0<br />tvOS = 13.0 |
</Card>

Internal release — no client-facing changes.
...
<Badge color="orange">Fixes:</Badge>\
Fixed an issue where the `typed throws` Swift API was not exported correctly in the module interface. No changes required in your project.
...
<Badge color="green">What's New:</Badge>\
Added support for initializing the default SDK instance using configuration data in `Info.plist`, as an alternative to calling `MoEngage.sharedInstance.initialize(...)` in code. Add the following keys under a `MoEngageConfiguration` dictionary in your `Info.plist`:

```xml
<key>MoEngageConfiguration</key>
<dict>
  <key>AppID</key>
...
For the full list of supported keys and values, refer to [SDK Initialization](/developer-guide/ios-sdk/sdk-integration/basic/sdk-initialization).
...
- **New required dependency:** InApps and RealTimeTrigger modules now depend on a Kotlin Multiplatform (KMP) shared library. See [New required dependency: Kotlin Multiplatform](#new-required-dependency-kotlin-multiplatform) below.

## New required dependency: Kotlin Multiplatform

Starting with 10.00.0, the InApps and RealTimeTrigger modules use a Kotlin Multiplatform (KMP) shared library for trigger condition evaluation. This replaces the previous platform-specific implementation with shared cross-platform logic, which ensures that campaign trigger conditions behave consistently across iOS and Android.

**Why this change?** Previously, the iOS and Android SDKs each maintained separate implementations of the trigger condition evaluator, which could produce different results for identical campaign configurations. Moving to a shared KMP implementation eliminates that inconsistency.

**What you need to do:**

- **SPM:** The KMP framework is pulled in automatically as a transitive dependency. No action required.
- **CocoaPods:** The KMP framework is included automatically via the updated podspec. Run `pod update MoEngage-iOS-SDK` (and `pod update MoEngageInApp` / `pod update MoEngageRealTimeTrigger` as applicable) to pick up the new dependency.
- **Manual / XCFramework:** You must add the new KMP-generated XCFramework to your Xcode project. Contact MoEngage support or refer to the manual integration guide for the list of new frameworks to add.

**App size impact:** The KMP shared library adds a native binary component compiled specifically for iOS/tvOS. Contact your MoEngage account team if you have questions about the size impact for your specific integration.

<Warning>
  If you use manual XCFramework integration or a custom build pipeline, you must add the new KMP-generated framework before upgrading to 10.00.0. Skipping this step will result in a linker error at build time.
</Warning>
...
<Badge color="green">What's New:</Badge>\
Added Kotlin Multiplatform (KMP) based event trigger condition evaluator. See [New required dependency: Kotlin Multiplatform](#new-required-dependency-kotlin-multiplatform) above.
